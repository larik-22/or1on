...
<svelte:head>
	<title>Deventer Guide</title>
	<link rel="icon" href="test.svg" type="image/png">
	...
	<div class="flex">
		{#if innerWidth < 640}
			<div class="sm:block hidden" class:showSideBar={showSideBar}>
				<Navigation bind:showSideBar={showSideBar} FullWidth={true} currentRoute={currentRoute}
							bind:currentPage={page}></Navigation>
			</div>
			<button type="button"
		...
		{:else}
			<Navigation bind:showSideBar={showSideBar} FullWidth={false} currentRoute={currentRoute}
						bind:currentPage={page}></Navigation>
		{/if}

		<!--        eslint-disable-next-line-->
		<svelte:component {params} this={page}/>
	</div>
